...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (isset($_FILES['foto_pegawai']) && $_FILES['foto_pegawai']['error'] === UPLOAD_ERR_OK) {
            ...
            $query = "SELECT foto FROM publikasi WHERE id = ?";
            ...
            $originalFileFullPath = __DIR__ . '../../../assets/img/blog/' . $originalFilePath;
            ...
            if (move_uploaded_file($fileTmpPath, $destPath)) {
                ...
                $stmt = $conn->prepare("UPDATE publikasi SET foto = ?, judul = ?, deskripsi = ?, deskripsi_singkat = ? WHERE id = ?");
                $stmt->bind_param("sssss", $newFileName, $_POST['judul'], $_POST['deskripsi'], $_POST['deskripsi_singkat'], $_POST['id']);
                if ($stmt->execute()) {
