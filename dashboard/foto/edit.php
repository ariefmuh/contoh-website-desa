...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (isset($_FILES['foto_pegawai']) && $_FILES['foto_pegawai']['error'] === UPLOAD_ERR_OK) {
            ...
            $uploadDir = __DIR__ . '../../../assets/img/foto-desa/';
            ...
            $query = "SELECT foto FROM foto_desa WHERE id = ?";
            ...
            $originalFileFullPath = __DIR__ . '../../../assets/img/foto-desa/' . $originalFilePath;
            ...
            if (move_uploaded_file($fileTmpPath, $destPath)) {
                ...
                $stmt = $conn->prepare("UPDATE foto_desa SET foto = ?, judul = ?, deskripsi = ? WHERE id = ?");
                $stmt->bind_param("ssss", $newFileName, $_POST['judul'], $_POST['deskripsi'], $_POST['id']);
                if ($stmt->execute()) {
