    ...
    if (!isset($_POST['csrf_token']) || $_POST['csrf_token'] !== $_SESSION['csrf_token']) {
        ...
        if (isset($_FILES['foto_pegawai']) && $_FILES['foto_pegawai']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($fileTmpPath, $destPath)) {
                ...
                $stmt = $conn->prepare("INSERT INTO publikasi (foto, judul, deskripsi_singkat, deskripsi) VALUES (?, ?, ?, ?)");
                $stmt->bind_param("ssss", $newFileName, $_POST['judul'], $_POST['deskripsi_singkat'] ,$_POST['deskripsi']);
                if ($stmt->execute()) {
